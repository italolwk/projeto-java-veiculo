...
public class JanelaPrincipal extends javax.swing.JFrame {
    ...
    private void btnPesquisarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPesquisarActionPerformed
        ...
        if (placa.isEmpty()) {
            txtArea.setText("Informe a placa para pesquisar.\n");
            return;
        }

        NoAVL no = avl.searchAVL(new Automovel(placa, "", 0, 0.0f));
        if (no == null) {
            txtArea.setText("Automóvel com placa " + placa + " não encontrado.\n");
        } else {
            txtArea.setText(no.getDado().toString() + "\n");
        }
    ...
    private void btnAtravessaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAtravessaActionPerformed
        // >>> ALTERADO: exibe os atravessamentos, incluindo o percurso em nível.
    ...
    private void btnInserirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnInserirActionPerformed
        try {
            ...
            float preco = Float.parseFloat(txtPreco.getText().trim().replace(',', '.'));

            if (placa.trim().isEmpty() || marca.isEmpty()) {
                txtArea.setText("Informe a placa e a marca do automóvel.\n");
            } else if (avl.searchAVL(new Automovel(placa, "", 0, 0.0f)) != null) {
                txtArea.setText("Já existe um automóvel cadastrado com a placa " + placa + ".\n");
            } else {
                avl.insereAVL(new Automovel(placa, marca, ano, preco));
                txtArea.setText("Automóvel inserido com sucesso.\n");
            }
        } catch (NumberFormatException erro) {
            txtArea.setText("Ano ou preço inválido. Confira os valores digitados.\n");
        }
        //Limpar os campos:
        txtPlaca.setText("");
        txtMarca.setText("");
    ...
    private void btnRemoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRemoverActionPerformed
        String placa = txtPlaca.getText().trim().toUpperCase();
        if (placa.isEmpty()) {
            txtArea.setText("Informe a placa para remover.\n");
            return;
        }

        boolean removido = avl.removeAVL(new Automovel(placa, "", 0, 0.0f));
        if (removido) {
            txtArea.setText("Automóvel com placa " + placa + " removido.\n");
        } else {
            txtArea.setText("Automóvel com placa " + placa + " não encontrado.\n");
        }
        txtPlaca.setText("");
