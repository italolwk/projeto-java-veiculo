...
public class AVL {
    ...
    private int compara(Object ob1, Object ob2) {
        return ((Comparable<Object>) ob1).compareTo(ob2);
    }
    ...
    // >>> INÍCIO DA ALTERAÇÃO: atravessamento em nível
    public void emNivel() {
        if (isEmpty()) {
            return;
        }

        LinkedList<NoAVL> fila = new LinkedList<>();
        fila.addLast(raiz);

        while (!fila.isEmpty()) {
            NoAVL noAux = fila.removeFirst();
            System.out.print(noAux.getDado() + ", ");

            if (noAux.getEsq() != null) {
                fila.addLast(noAux.getEsq());
            }
            if (noAux.getDir() != null) {
                fila.addLast(noAux.getDir());
            }
        }
    }

    // >>> MÉTODO ALTERADO: versão que retorna o atravessamento como String
    public String emNivelString() {
        ...
        while (!fila.isEmpty()) {
            NoAVL noAux = fila.removeFirst();
            resp.append(noAux.getDado()).append('\n');
